...
public class UiManager : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        if (SceneManager.GetActiveScene().name == "SelectLevelScene")
        {
            ...
            if (PlayerPrefs.GetInt(StringManager.currentLevel) == 0)
                PlayerPrefs.SetInt(StringManager.currentLevel, 1);
            //int currentLevel = PlayerPrefs.GetInt(StringManager.currentLevel,1);
            UpdateAllStars();
            UpdateBlueLevelButton();
            ShowButtonStatus();
        }
